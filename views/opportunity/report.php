    ...
    <tbody>
        <?php foreach($entity->sentRegistrations as $r): ?>
            <tr>
				...
				<td><a href="<?php echo $r->singleUrl; ?>" target="_blank"><?php echo $r->singleUrl; ?></a></td>
                ...
                foreach($custom_fields as $field) {
					...
					if ($field['type'] === 'cpf'){
						echo preg_replace("/(\d{3})(\d{3})(\d{3})(\d{2})/", "\$1.\$2.\$3-\$4", $_field_val);
					}else if ($field['type'] === 'cnpj'){
						echo preg_replace("/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/", "\$1.\$2.\$3/\$4-\$5", $_field_val);
					}else{
